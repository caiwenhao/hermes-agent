        ...
        if _preserve_unsigned_thinking:
            ...
            for b in m["content"]:
                ...
                if b.get("type") == "redacted_thinking":
                    ...
                    new_content.append(b)
            # Seed an explicit empty thinking block for poisoned history:
            # older Hermes sessions may have persisted assistant text or
            # tool-use turns before provider thinking state was captured.
            # sub2api requires every prior assistant turn in thinking mode to
            # replay a ``content[].thinking`` block, so prepend an empty one
            # when none survived above.
            _has_thinking = any(
                isinstance(b, dict) and b.get("type") in _THINKING_TYPES
                for b in new_content
            )
            if not _has_thinking:
